...
import type { Collaborator } from '@/types/APIContextTypes';
import Dialog from '@/components/Dialog';

const Card = ({ collaborator }: { collaborator: Collaborator }) => {
  const handleDelete = () => {
    // Lógica para deletar o colaborador
    console.log(`Deletando colaborador: ${collaborator.name}`);
    // Aqui você deve chamar a função que realmente deleta o colaborador
    // Provavelmente você vai querer passar esta função como prop do componente pai
  };

  return (
    <Dialog
      title='Confirmação de Exclusão'
      body={`Tem certeza que deseja excluir o colaborador ${collaborator.name}?`}
      confirmText='Deletar'
    ...
    >
      {({ openDialog }) => (
        <div
          className='flex flex-col items-center max-96 h-60 rounded-xl shadow w-11/12'
          id={collaborator.id.toString()}
        >
          <div className='bg-white flex h-16 items-center justify-between px-4 rounded-t-xl w-full'>
            <div className='flex gap-3 items-center'>
              <div
                className={`grid h-8 place-items-center rounded-full w-8 ${
                  collaborator.position.description === 'Subordinado'
                    ? 'bg-jade-100'
                    : 'bg-sail-100'
                }`}
              >
